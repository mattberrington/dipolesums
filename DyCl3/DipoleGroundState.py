...
class Lattice:
	...
	def Ion1Position(self,x,y,z):
		...
		self.Position['4A'] = self.a + self.b + self.ion1
		self.Position['5A'] = self.c + self.ion1
	    ...
	    with open('BMatrixValuesDyCl3New.csv') as csvfile:
	        ...
	        for row in spamreader:
	            if n == 0:
	                ...
	                self.B[m][0,0]=row[0]*self.gx*self.gx*factor
	                self.B[m][0,1]=row[1]*self.gx*self.gy*factor
	                self.B[m][0,2]=row[2]*self.gx*self.gz*factor
	                n+=1
	            elif n == 1:
	                self.B[m][1,0]=row[0]*self.gy*self.gx*factor
	                self.B[m][1,1]=row[1]*self.gy*self.gy*factor
	                self.B[m][1,2]=row[2]*self.gy*self.gz*factor
	                n+=1
	            else:
	                self.B[m][2,0]=row[0]*self.gz*self.gx*factor
	                self.B[m][2,1]=row[1]*self.gz*self.gy*factor
	                self.B[m][2,2]=row[2]*self.gz*self.gz*factor
	                n=0
	                m+=1
