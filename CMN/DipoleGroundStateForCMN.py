...
class Lattice:
	def __init__(self,a_len,b_len,c_len,beta):
		self.a = np.array([+11.0/np.sqrt(3)*(np.sqrt(3)/2),11.0/np.sqrt(3)*(1/2),17.3/3])
		self.b = np.array([-11.0/np.sqrt(3)*(np.sqrt(3)/2),11.0/np.sqrt(3)*(1/2),17.3/3])
	...
	def WriteLatticeSums(self,SumRadius):
	
		with open('MatrixValuesCMNNew.csv', 'w') as csvfile:
			writer = csv.writer(csvfile, lineterminator = '\n')
			
			verticesbase = self.LatticePoints(SumRadius)

			
			for key in np.arange(1,9): 
				
				jVector = self.Position(key)
				
				vertices = self.ShiftAndTrim(verticesbase,SumRadius,1,key)				
				
				# x = vertices[:,0] - jVector[0]				
				# y = vertices[:,1] - jVector[1]
				# z = vertices[:,2] - jVector[2]
				# r = np.sqrt(np.sum(np.power(vertices - jVector,2),axis=1))
...
CMN = Lattice(0,0,0,0)
CMN.Ion1Position(0,0,0)
CMN.gTensor(1.84,1.84,0)
CMN.WriteLatticeSums(400)
#SI UNITS
mB=9.274*10**(-24) 
k=1.380*10**(-23)
NA=6.022*10**23 
