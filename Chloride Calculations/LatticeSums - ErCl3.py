...
def cell_translation_vectors(a,b,c,beta):
    ...
    vect_a = Ry(167*pi/180) @ np.array([a,0,0])
    vect_b = Ry(167*pi/180) @ np.array([0,-b,0])
    vect_c = Ry(167*pi/180) @ Ry(-beta) @ np.array([c,0,0])
...
def WriteLatticeSums(letter,jPositions,iPosition,SumRadius):
    with open(path+letter+'MatrixValuesErCl3.csv', 'w') as csvfile:
...
def CreateAMatrix(gx,gy,gz,factor,demagxx,demagyy,demagzz):
    with open(path+'AMatrixValuesErCl3.csv') as csvfile:
...
def CreateAMatrixPaper():
    with open(path+'AMatrixValuesErCl3PaperValues.csv') as csvfile:
        spamreader = csv.reader(csvfile,quoting=csv.QUOTE_NONNUMERIC)
        global APaper
        APaper = {}
        n = 0
...
def CreateBMatrixPaper():
    with open(path+'BMatrixValuesErCl3PaperValues.csv') as csvfile:
        spamreader = csv.reader(csvfile,quoting=csv.QUOTE_NONNUMERIC)
        global BPaper
        BPaper = {}
        n = 0
...
def CreateBMatrix(gx,gy,gz,factor,demagxx,demagyy,demagzz):
    with open(path+'BMatrixValuesErCl3.csv') as csvfile:
        spamreader = csv.reader(csvfile,quoting=csv.QUOTE_NONNUMERIC)
        global B
        B = {}
        n = 0
...
with open(path+'/EigValsErCl3.csv', 'w') as csvfile:
    writer = csv.writer(csvfile, lineterminator = '\n')  
    for key in eigvals:                   
