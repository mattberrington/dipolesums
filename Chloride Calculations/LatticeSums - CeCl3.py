...
path = 'F:/Dropbox/Dropbox/university/Ordering Temperature Model/Chloride Calculations/'
...
#define the translation vectors for the unit cell, should work for a general triclinic crystal
...
def cell_translation_vectors(a,b,c,beta):
    ...
    vect_a = Ry(159*pi/180) @ np.array([a,0,0])
    vect_b = Ry(159*pi/180) @ np.array([0,-b,0])
    vect_c = Ry(159*pi/180) @ Ry(-beta) @ np.array([c,0,0])
...
def WriteLatticeSums(letter,jPositions,iPosition,SumRadius):
    with open(path+letter+'MatrixValuesCeCl3.csv', 'w') as csvfile:
...
def CreateAMatrix(gx,gy,gz,factor,demagxx,demagyy,demagzz):
    with open(path+'AMatrixValuesCeCl3.csv') as csvfile:
...
def CreateAMatrixPaper():
    with open(path+'AMatrixValuesCeCl3PaperValues.csv') as csvfile:
        spamreader = csv.reader(csvfile,quoting=csv.QUOTE_NONNUMERIC)
        global APaper
        APaper = {}
        n = 0
...
def CreateBMatrixPaper():
    with open(path+'BMatrixValuesCeCl3PaperValues.csv') as csvfile:
        spamreader = csv.reader(csvfile,quoting=csv.QUOTE_NONNUMERIC)
        global BPaper
        BPaper = {}
        n = 0
...
def CreateBMatrix(gx,gy,gz,factor,demagxx,demagyy,demagzz):
    with open(path+'BMatrixValuesCeCl3.csv') as csvfile:
        spamreader = csv.reader(csvfile,quoting=csv.QUOTE_NONNUMERIC)
        global B
        B = {}
        n = 0
...
cell_translation_vectors(9.57,6.47,7.84,93.65*pi/180)
# print(vect_a)
# print(vect_b)
# print(vect_c)
...
with open(path+'/EigValsCeCl3.csv', 'w') as csvfile:
    writer = csv.writer(csvfile, lineterminator = '\n')  
    for key in eigvals:                   
